...
class ZeroDel_FisherZ(CIT_Base):
    ...
    def __call__(self, X, Y, condition_set=None, return_rho=False, return_sample_size=False):
        '''
        Perform an independence test using Fisher-Z's test for data with missing values (zero values for dropout data).
        Data rows where zeroes/missing values are present in any of the **conditioning variables**
            (in the specific case for the dropout model) are deleted.
        You may also try to delete rows where zeroes/missing values are present in any of the **involved variables**
            (i.e., X, Y, and the conditioning vars) by editing the following line: `test_wise_deletion_rows_index = self._get_index_no_mv_rows(var)`.
            This is still correct (according to Def 2 and Thm 2 in the paper), though it may be less powerful with fewer sample size.
        Parameters
        ----------
        X, Y and condition_set : column indices of data
        ...
        test_wise_deletion_rows_index = self._get_index_no_mv_rows(condition_set)    # or use self._get_index_no_mv_rows(var)
        N_test_wise_deleted = len(test_wise_deletion_rows_index)
        ...
        test_wise_deleted_data_var = self.data[test_wise_deletion_rows_index][:, var]
        sub_corr_matrix = np.corrcoef(test_wise_deleted_data_var.T)
        inv = np.linalg.pinv(sub_corr_matrix)   # use pinv to avoid singular matrix error; any other way to calculate this partial correlation?
        r = -inv[0, 1] / sqrt(inv[0, 0] * inv[1, 1])
        if abs(r) >= 1: r = (1. - np.finfo(float).eps) * np.sign(r)  # may happen when samplesize is very small or relation is deterministic
        Z = 0.5 * log((1 + r) / (1 - r))
...
class ZeroDel_KCI(CIT_Base):
    ...
    def __call__(self, X, Y, condition_set=None, return_sample_size_for_debug=False):
        ...
        test_wise_deletion_rows_index = self._get_index_no_mv_rows(condition_set)
        if len(test_wise_deletion_rows_index) > self.max_sample_size:
            test_wise_deletion_XYcond_rows_index = random.sample(test_wise_deletion_rows_index, self.max_sample_size)
        ...
        subdata = self.data[test_wise_deletion_rows_index][:, var]
